...
// The test is output-interpolations.test.mjs. Run `node scripts/test/sweeps/output-interpolations.mjs [--all]`
...
/** The repository root: THREE directories above this file (scripts/test/sweeps/), wherever the
 *  checkout lives. It was two while this module lived directly in scripts/test/; 5.1 moved it into
 *  the sweep bucket with its test, and a root one level short would have had every sweep read a
...
export function sweepInterpolations({ repo = REPO, read = (rel) => fs.readFileSync(path.join(repo, rel), "utf8"), judged = JUDGED } = {}) {
  ...
  const where = "(declared in scripts/test/sweeps/output-interpolations.judged.mjs)";
