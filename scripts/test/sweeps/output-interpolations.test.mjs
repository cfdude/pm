...
test("every interpolation into engine output is escaped, literal, sunk or judged — no UNCLASSIFIED, STALE, EXCESS or WIDE", () => {
  ...
  assert.deepEqual(findings, [], "escape each UNCLASSIFIED value, or judge it with its reason in scripts/test/sweeps/output-interpolations.judged.mjs; " +
    "fix or remove each STALE / EXCESS / WIDE judgment there (run `node scripts/test/sweeps/output-interpolations.mjs` to list them)");
});
