...
// RE-POINTED BY 5.1: the seam is the shared-`fs` patch (`injectConflictOnce`), not the NODE_OPTIONS
// preload. A preload only reaches a process that is STARTED, and the assertion half no longer starts
// one — the engine is invoked in the running process. The patch is the same one-shot bump of the
// on-disk revision at saveState's first mkdir, and the restore is called in a `finally` because `fs`
// is shared with every other file in the half.
...
test("gh-131: a hook write that conflicts on its first attempt still lands the heal — render", () => {
  ...
  let fired;
  try { run(["render"], { cwd }); } finally { fired = restore(); }
