...
//
// 4.1 (0.48.0) moved FIFTEEN of this file's nineteen tests to the unit rung
// (`scripts/test/unit/conductor-29.test.mjs`): the union-of-bands invariant, the whole write-path
// family and the whole gh#94 deferral family. Their observables are all values — a refusal message,
// a JSON payload, the rendered PROJECT.md or the record's own bytes.
//
// WHAT REMAINS IS THE SOURCE-READING HALF, and this half is where a source read belongs: gh#100's
// own `rg 'KNOWN_[A-Z_]+ =' constants.mjs` reproduction, the two drift guards (one walks every
// engine source file, one opens each file a declaration claims) and `commands/epic.md`. They are
// the direction the issue was filed about — the set must not stop describing the code — and each
// of them reads a path the store does not own.
...
import {
  KNOWN_LINK_TYPES, LINK_TYPES_READ, LINK_TYPES_WRITTEN,
  isKnownLinkType,
} from "../../lib/links.mjs";
