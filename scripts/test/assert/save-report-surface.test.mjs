...
// `new URL("../", …)` while this file lived at scripts/test/, where `../` IS scripts/; the move into
// scripts/test/assert/ silently turned it into scripts/test/, a directory with no .mjs in it at all.
// The walk's own `>= 20` floor did NOT catch it — lib/ alone clears twenty — so this was a guard
// failing open on exactly the "absent edits" class it exists for.
