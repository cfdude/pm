...
import { tmpRepo, run, runCombined, readState, writeState, projectMd, parseBrief, expectFail, invokeEngine } from "../fixtures/assert-harness.mjs";
...
test("the engine banner is shown by default when CLAUDE_PROJECT_DIR is NOT set (outside a dev/self-hosting context), so a stale cached engine is still visible there", () => {
  ...
  const r = invokeEngine(["render"], { cwd, env: { CLAUDE_PROJECT_DIR: undefined } });
