...
export function recordReconcile() {
  ...
  if (!KNOWN_RECONCILE_VERDICTS.includes(verdict)) {
    die(`conductor: --verdict must be one of ${KNOWN_RECONCILE_VERDICTS.join("|")}\n`);
  }
  ...
  const refuse = (why) => { die(`conductor: ${why} — ${owedPhrase}. Nothing was written.\n`); };
  ...
  if (!state.epics.some(e => e.id === detourId)) {
    die(`conductor: detour epic '${escapeControls(detourId)}' not found\n`);
  }
