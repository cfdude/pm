...
- **A blank value is refused, not read as an absent one.** `activity --since "   "`,
  `activity --epic "   "`, `purge-logs --kind "   "` and the rest exit non-zero and change
  nothing. These three verbs go through the same `parseFlags` + `requireFlagValues` path as every
...
- **An undeclared flag is refused by name.** `activity --bogus`<!-- pm:refused unknown-flag --> exits non-zero rather than
  ...
  `set-activity-log on extra`<!-- pm:refused extra-positional --> is refused too. `--help` anywhere after the verb prints help and
