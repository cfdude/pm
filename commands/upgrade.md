...
4. **Act on any OpenSpec drift line the command printed.** `pm` and `superpowers` are plugins and
   ...
   than the one installed, the command prints a line naming both versions. **`pm` never runs
   `openspec update` for you** — it is a terminal command, exactly like `openspec init`. What to do
   with the line:
   ...
   - **It says the generated files are git-tracked here.** Run `openspec update` in a terminal,
     then read `git diff` before committing. That diff is the review — the update rewrites files
