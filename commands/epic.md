...
| `--withdraw-gate-review <1\|2>` | `gateReview`, `withdrawnGateReviews` | Withdraws a recorded gate verdict that does not belong on this epic, and records why with `--withdrawal-reason`. The WHOLE entry moves, `superseded` included, into `withdrawnGateReviews` — recorded, never erased. |
