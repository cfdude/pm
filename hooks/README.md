...
lesson advisor — surfaces a docs/lessons/ entry whose `detect:` matcher matches the pending tool call, BEFORE the mistake. ADVISORY ONLY: it never blocks and always exits 0, which is why it is a separate entry from the gate guard above. A SEPARATE, WIDER matcher on purpose: half the matchable lessons match on a COMMAND, so Bash must be covered or they are dead on arrival. Silent in projects that have not run /pm:init, and in any project with no docs/lessons/. A `detect:` that cannot work — not a JSON object, an unknown key, a tool this matcher never sends, no positive predicate, a regex that does not compile — is REJECTED by the engine's classifier (`classifyLessons()`), and the hook stays silent about it: rejects are reported outside the hook path, by pm's own per-commit `lessons-index` test (a consumer-facing report is cfdude/pm#228). Each regex runs under its OWN 50 ms budget, capped at 1 s per hook call, against at most 4096 characters of the command's first line; one that runs out of time counts as not matched, so a runaway lesson costs only its own advice.
