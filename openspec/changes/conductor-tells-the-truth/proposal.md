...
- **BREAKING** The archive transition is gated on **every** path, not just `update-epic`. `reconcileArchived()` currently flips an epic to `archived` with no lane check and no gate check, and it runs from the SessionStart, PreCompact and PostToolUse hooks — so the transition happens before the operator's next prompt. Archives that today succeed silently will refuse, or record how they bypassed.
