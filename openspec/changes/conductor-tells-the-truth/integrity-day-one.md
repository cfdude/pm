...
| `gate-recorded-as-bookkeeping` | **Arm 1 finds nothing**: it needs BOTH a merge commit (the last hash in an epic's attribution array) and a verdict dated after it. As measured on 2026-08-23 no epic had attributed anything at all. Since then `conductor-tells-the-truth` has attributed its own commits — the release exercising its own obligation — and holds no gate verdict, so the comparison has a population and still yields nothing. (`gh-132-lessons-advisor-hook` carries an *empty* array because it was created under this release; empty names no merge commit.) The assertion behind this zero is stated as that relation rather than as the count, which stopped being 0 the moment the record became more complete. **Arm 2 finds nothing**: exactly one epic holds two gate verdicts, and it recorded them **22 minutes apart** — a spec review and an implementation review, which is what the arm's 60-second bound is calibrated to distinguish from bookkeeping. |
