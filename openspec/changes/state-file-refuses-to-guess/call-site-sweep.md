...
| remover | `releaseStateLock()` (only our inode+nonce); `breakStaleLock()` (only the judged identity, still stale; and a break file older than the stale age whose inode is unchanged) |

Classifiers: `purge-logs` kinds are activity/conflicts/detours logs — the lock is not a log and needs no entry. `isConductorOwnFiles` classifies files in a commit's diff; the lock paths are gitignored (`.conductor/state.json.lock*`) and live only for one save, so they cannot appear there. `VERB_EFFECTS` `writes` strings describe lasting effects; the lock is removed on every return and throw (3.2 guard), so no string gains it. The detached-HEAD table: not suppressed (3.3 detached case). Pre-existing and unchanged: `state.json.tmp-<pid>-<ts>` is left behind only by a process killed between its write and its rename; it is not gitignored. The new ownership-refusal path unlinks its temp file.
...
| `readRepoClaim()` / `writeRepoClaim()` / `clearRepoClaim()` | `claim --repo`, `unclaim --repo`, `owners` | The writer is temp file plus rename (5.5); not locked (D5). A killed writer can leave `session-claim.json.tmp-*`, which the `.conductor/session-claim.json*` entry ignores. |
