...
| `argv-surface.mjs` `checkedPositionals()` (was a local in `subcommands.mjs:28`; exported by the Gate 2 follow-up) | the check's positional list, for `log-detour`, `honcho-memory` (`subcommands.mjs:680,729`) and the four place-readers below | Holds: every verb that reads a positional by place reads classified positionals, never the argv tail (D10). |
...
| `gate-guard.mjs:20` (`set-gate-guard`), `activity-log.mjs:66` (`set-activity-log`), `lane-routing.mjs:105` (`suggest-lane`), `triage.mjs:156` | the check's first classified positional (`checkedPositionals()`) — was `process.argv[3]` | Holds since the Gate 2 follow-up. **The `argv[3]` read did NOT hold** (Gate 2, spec: "an argv-level flag SHALL NOT be read as a positional"): with no positional the canonical rewrite leaves `--force` at `argv[3]`, so `set-gate-guard --force` exited 1 with usage where bare `set-gate-guard` reads the guard (exit 0). `set-activity-log --force` read `--force` as its value too, and only happened to print the same usage as its bare form, which has no read branch. `suggest-lane` and `triage` are read-only, so `--force` is refused before they run; they read the classification anyway so a later argv-level row on a read-only verb cannot reopen the defect. Guarded by the verb-surface test "An argv-level flag with no positional is not read as one". **Behaviour change outside the spec's scenarios:** a positional written after flags is read (`triage --limit 3 "ask"` answers where it printed usage). gh-186's guard still holds — `triage --limit 5` has no positional, so nothing is read and usage is printed. |
| `argv[0]` readers above, `rank.mjs:61`, `claims.mjs` `positionalArgs()` | swept again for the same defect | Hold: each skips a `--`-leading token, so `--force` with no positional yields no positional and the verb's own minimum-positional refusal answers exactly as the bare form does. |
...
| `checkedPositionals(` | `subcommands.mjs:680,729` (log-detour, honcho-memory), `gate-guard.mjs:20`, `activity-log.mjs:66`, `lane-routing.mjs:105`, `triage.mjs:156` | Holds: the only reader of a positional by place; `rg -n "process\.argv\[3\]" scripts` matches only comments after the Gate 2 follow-up. |
