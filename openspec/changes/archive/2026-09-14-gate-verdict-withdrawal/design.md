...
|---|---|---|
| `deliveredObligations` (Gate 2, `delivered`, openspec) | `!gate2` → fails | Its Gate 2 `detail` names the withdrawal; the reason goes in `items` for each renderer to quote. Every caller (the archive gate and the regression check) inherits it |
