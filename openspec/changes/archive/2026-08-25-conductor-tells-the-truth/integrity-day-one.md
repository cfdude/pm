...
| `archived-with-withdrawn-gate-2` | **No epic here has a Gate 2 in the withdrawn state.** Added 2026-09-14 for 0.43.0 (`gate-verdict-withdrawal`): the withdrawn kind of the same standing condition, reported under its own id because the sibling's "no review from anyone" is false of a review that was recorded and then taken back. It names an archived, openspec-lane epic in completion scope whose `gateReview.gate2` is absent and whose `withdrawnGateReviews` holds a Gate 2 entry. The zero is "nothing has been withdrawn yet", not "withdrawals are being handled": the first `update-epic <id> --withdraw-gate-review 2` run on a LIVE archived epic of this repository puts it in front of this check, and that finding must then be explained here — the #192 shape, where an unexplained live finding turned this document's own test red. || `self-referential-epic-id` | **No stored epic id in this record names the epic that holds it.** Added 2026-09-17 for 0.46.0 (`operations-ship-their-inverses`), reading the same `epicReferences()` declaration as `dangling-epic-reference` — which cannot see this shape at all, because a self-reference names an epic the record demonstrably DOES hold. The zero is local and it is not the general case: the finding was reproduced against the 0.45.0 engine in a hermetic fixture, where `update-epic s1 --status archived --outcome delivered --carried-to s1 --no-deferrals` exited **0** and SATISFIED the handoff obligation, and `integrity` said nothing. That write is now refused, so the zero here is measured against the refusal as well as against the record. |
| `empty-epic-id` | **No stored epic id in this record is an empty string.** Added 2026-09-17 for 0.46.0. Before that release the check could not have fired even against a record holding one: `epicReferences()`'s `add()` emitted a holder only where the value was a non-empty string, so an empty id never entered the set any check reads — the declaration became value-agnostic in the same change, with each consumer keeping its own predicate so one defect is still reported by exactly one check. The live instance it was written from was `--deferral ":"` storing `{epic:"",section:""}` on the 0.45.0 engine, exit 0, with `integrity` reporting only the unrelated ghost id beside it. |
| `grant-names-nothing` | **No epic here holds an autonomy grant with neither an action nor a category.** Added 2026-09-17 for 0.46.0. It is the one shape in this table that NO remedy can reach and that is deliberate: a revoke names a stored value and an empty one is not expressible as a flag value, and a grants clear-all is deletion, which `epic-autonomy` rules out — so the report is the only surface that sees it, and it prints no invocation. Measured on 0.45.0: `set-autonomy a1 --preauthorize ":no action"` exited 0 and stored `{"action":"","grantedAt":…,"reason":"no action"}`, a grant matching nothing or everything depending on how a reader implements the match. That write is now refused. |
