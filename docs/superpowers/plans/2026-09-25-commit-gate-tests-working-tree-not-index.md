...
| `.githooks/pre-commit` → `scripts/test/drift.mjs` | **Yes — after the branch review** | **CORRECTED.** This row first said "drift's own reads are already index-only". That was false: its GIT calls were, but every SET it judges (certified modules, engine-source files, functional/assertion/sweep ids, conformance rows) came from `fs` reads of the working tree (`certification.mjs` `certifiedModules`, `engineSourceFiles`, `testIdsIn`, `conformanceRows`). The review reproduced it: a gateway-calling `scripts/lib/zz-probe.mjs`, staged and then deleted from disk, passed drift. Now `checkAll` passes `indexReaders(root)` — `ls-files -z` + `show :<path>` under the handed index — into all of them; only the certification record is read from disk (IX-j, IX-i). The hook also runs the SNAPSHOT's drift.mjs (IX-k). |
| `invokedDirectly` in `drift.mjs`, `certify.mjs`, `node-majors.mjs` | **Yes — all three** | `rg -n invokedDirectly scripts/test/*.mjs` → 3 sites, one shape. Compared `path.resolve(argv[1])` with the module's URL path, but Node resolves the main module to its realpath, so a script run through a symlinked directory silently did nothing and exited 0 — found when drift first ran from the snapshot under macOS `$TMPDIR`. All three compare realpaths now. |
