...
| [`squash-merge-orphans-the-evidence`](squash-merge-orphans-the-evidence.md) | About to squash-merge a PR in a repo whose records reference commit shas — an attribution array, a review verdict's range, a doc citing a fix. | Tag the pre-squash tip and push it BEFORE merging. Squash makes the branch's commits reachable from nothing, and `gc` deletes them two weeks later, from every clone. | 🔔 |
...
| `squash-merge-orphans-the-evidence` | .claude/skills/pr-workflow/SKILL.md steps 5 and 8; CONTRIBUTING.md § Branch workflow |
