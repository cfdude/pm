...
| [`a-guard-installed-by-import-covers-only-its-process`](a-guard-installed-by-import-covers-only-its-process.md) | Before changing how a test runner isolates files — dropping a single-process flag, raising concurrency, sharding — list every guard installed as a side effect of an import (a PATH shim, an exit listener, a monkey-patched module, a counter armed at load). | A guard installed by an import covers the PROCESS that imported it, never "the suite". When isolation changes, enumerate every side-effect install, then make each file install what it relies on itself and add a walk that refuses a file which does not. Per-process resources a guard creates must be removed per process. | — |
| [`a-guard-is-only-as-wide-as-the-events-it-is-wired-to`](a-guard-is-only-as-wide-as-the-events-it-is-wired-to.md) | You are writing, reviewing or relying on a mechanical block — a hook matcher, a middleware route list, a CI path filter, an allowlist of subscribed events — and its message or its docs describe an OBLIGATION rather than the events it is wired to. | A guard's reach is the set of events it is SUBSCRIBED to, never the set of actions its message forbids. Read the subscription from the shipped configuration, then name one action that reaches the forbidden outcome through an unsubscribed event. | — |
| [`a-one-off-sweep-certifies-only-the-day-it-ran`](a-one-off-sweep-certifies-only-the-day-it-ran.md) | You are running a sweep inside a change — a script under the change directory or a scratchpad, never added to the suite — over a whole class of output (every emitted command, every doc invocation, every call site), and you are about to record its zero-findings result as a claim about the product. | A sweep over a class of output is a test, so make it one: put it in the suite, where every later edit of that class re-runs it. State a zero-findings result as exactly what it checked, never as the property it was run for. | — |
...
| [`a-static-guard-over-dynamic-code-needs-declared-limits`](a-static-guard-over-dynamic-code-needs-declared-limits.md) | You are writing a static checker that reads SOURCE — a lexer, a regex sweep, an AST pass — to prove a property holds at every site in a dynamic language, and you intend to report "0 findings" without saying which shapes it cannot see. | A static guard over a dynamic language is best-effort by construction: ship it with the unsound shapes written down beside it (each verified absent today, with the command that checked) and with a behavioural backstop that exercises the real product. Report what it lexes, never the property it was built for. | — |
| [`a-surviving-mutant-can-be-a-second-mechanism-not-a-weak-test`](a-surviving-mutant-can-be-a-second-mechanism-not-a-weak-test.md) | You are mutation-checking a guard whose write is followed, in the same command, by a self-healing or normalising pass over the same field — a render, a reconcile, a recompute, a migration, a formatter. | When a mutant survives, first ask WHO ELSE writes that field on the same code path — not whether the test is weak. Where a downstream heal repairs what the mutation broke, assert on the mutated command's OWN output, not on the end state. | — |
| [`a-subagent-that-backgrounds-its-run-ends-without-a-report`](a-subagent-that-backgrounds-its-run-ends-without-a-report.md) | You are writing the dispatch prompt for a subagent — a gate reviewer above all — whose work includes a long command (the full test suite, a fixture build, a Layer B run) that it may choose to start in the background. | Tell every dispatched subagent its turn ending is its report: run long commands in the foreground, redirected to a file and read back, and never end the turn while one is running. A reply that says it is waiting is a failed dispatch — re-dispatch it. | — |
| [`an-archive-writes-outside-the-change-dir`](an-archive-writes-outside-the-change-dir.md) | You have just run `openspec archive` and are about to stage or commit its result, or you are about to hard-reset a tree where an archive ran and was not committed. | An archive is one move with two halves: the change dir AND every main spec it rewrote. Stage `openspec/` whole (or everything `git status --short openspec/` lists), never `openspec/changes` alone; no hard reset while `openspec/specs/` paths are dirty. | — |
...
| [`explaining-a-cost-needs-a-control-not-a-story`](explaining-a-cost-needs-a-control-not-a-story.md) | You are about to write down, in a proposal, a design document or a commit message, the REASON a suite, a build or a pipeline is slow — before running the control that removes the term you are blaming. | Before explaining a cost, remove the term you are blaming and measure the difference. One syscall, one layer, nothing else changed — if the number does not move, the story is wrong. A plausible decomposition ("it does six round trips per test") is not evidence; it is a hypothesis, and it is the one most likely to be believed because it is detailed. | — |
...
| [`fixtures-the-product-should-refuse`](fixtures-the-product-should-refuse.md) | You are writing a fixture for a field that names something outside the record (a sha, an epic id, a detour) and type a placeholder because the verb accepts it today. | Create the real thing instead; if that is awkward, the verb may be accepting what it should refuse — write the refusal test first. | — |
...
| [`git-rewinds-restore-tracked-conductor-state`](git-rewinds-restore-tracked-conductor-state.md) | You are writing a test fixture that records a conductor write (an attribution, a status, a gate verdict) and later in the same fixture runs `git reset --hard`, `git checkout -f`, `git checkout --orphan` or any other command that rewrites the working tree. | `.conductor/state.json` is TRACKED, so a working-tree rewind restores the committed copy and drops every uncommitted conductor write. Record conductor state AFTER the last git rewind in a fixture, or commit it first — and assert the state you depend on (`attributedCommits` holds the sha) immediately before the step under test. | — |
...
| [`narrowing-a-guard-retests-what-it-protected`](narrowing-a-guard-retests-what-it-protected.md) | You are fixing a false positive in a guard by narrowing when it fires, and your test proves the false positive is gone. | List what the broad guard was stopping before you narrow it and keep a test for each; the fix's own test only proves the case you narrowed for. | — |
...
🔔 = carries a `detect:` matcher **the engine accepts**, so pm's `lesson-advice` `PreToolUse` hook surfaces it automatically. The rest are retrieval-only by design, because a matcher that fires wrongly is worse than none. `scripts/test/assert/lessons-index.test.mjs` holds this column equal to the engine's own classifier (`classifyLessons()` in `scripts/lib/lessons.mjs`) on every commit.

**A `detect:` that cannot work fails the commit that wrote it.** Until
[#194](https://github.com/cfdude/pm/issues/194), six of the twelve matchers here had never fired:
...
which no `detect` key can see (it reads `tool`, the path, and the command's first line only), so
their matchers were removed and they are retrieval-only.
...
| `a-guard-installed-by-import-covers-only-its-process` | scripts/test/assert/assert-half-has-no-spawn.test.mjs — the "2.1 every file in both rungs installs the git shim itself" walk; scripts/test/assert/git-shim.test.mjs for the per-process removal. Beyond the git shim, a habit. |
| `a-guard-is-only-as-wide-as-the-events-it-is-wired-to` | scripts/test/gate-guard-write-paths.test.mjs — the shipped hooks.json matcher is asserted to cover every tool the block claims, and the block message is asserted to state the obligation while no longer claiming to be the only way through |
| `a-one-off-sweep-certifies-only-the-day-it-ran` | scripts/test/emitted-invocations.test.mjs — the permanent Layer A/B/C sweep that replaced the one-off. Beyond that class, a habit; no mechanism notices a sweep left out of the suite. |
| `a-silent-noop-edit-reports-success` | habit — assert-the-match-count; no mechanism. The nearest mechanical cousin is the read-back verification `update-epic` performs after writing. detect matcher on an in-place `sed -i`; a `.replace(` in a script stays a habit. |
| `a-static-guard-over-dynamic-code-needs-declared-limits` | the KNOWN LIMITS block at the top of `scripts/test/output-interpolations.mjs`, shape by shape with the rg showing each absent from the engine; the behavioural backstop is section 7 of `scripts/test/output-text-integrity.test.mjs` (the registry-derived runtime poison sweep) plus the in-suite reviewer mutants of `output-interpolations.test.mjs` |
| `a-surviving-mutant-can-be-a-second-mechanism-not-a-weak-test` | habit; scripts/test/detour-frame-drop.test.mjs's "3.6: a render after the drop neither re-arms nor re-clears anything" asserts the drop's OWN stderr carries no heal line, as the worked example |
| `a-subagent-that-backgrounds-its-run-ends-without-a-report` | habit — a line in the dispatch brief; no mechanism |
| `an-archive-writes-outside-the-change-dir` | the release-checklist skill's step 5 — the archive commit stages `openspec/` whole (0.49.0, task 7.8) |
| `an-unused-active-pointer-turns-a-true-check-into-noise` | Nothing mechanical. `integrity.mjs`'s own comment states the heuristic and names this exact failure; the gap is operator behaviour, which is why this is a lesson and not a guard. detect matcher on the engine command that adds a release member or archives an epic. |
| `ascii-fixtures-hide-a-lossy-decode` | scripts/test/commit-observation.test.mjs "G2-C1 a reflog line that is not valid UTF-8 anchors by its bytes"; design.md Decision 2 (the record stores `anchor.lineBase64`). Beyond this code path, a habit — no mechanism finds a lossy decode. |
| `bind-rules-to-functions-not-enumerations` | required task 16.1 of this release; issue #115 |
| `cite-a-symbol-not-a-line-number` | Nothing mechanical — a line number is well-formed text and no test can tell a fresh one from a rotted one. This is a habit, which is why it is a lesson. detect matcher on a `--reason`/`--notes`/`--description` value citing `file.ext:N`. |
...
| `explaining-a-cost-needs-a-control-not-a-story` | task 0.3 of unit-rung-and-fixture-snapshots (0.48.0): baseline-before.md records the numbers at both ends, and `noop-fsync.cjs` is committed beside it so the control is re-runnable rather than quoted; task 6.5 re-runs it on the final commit. The rule is enforced by the shape of the task, not by a hook — nothing mechanically stops a proposal from asserting a cause. |
...
| `fixtures-the-product-should-refuse` | scripts/test/helpers.mjs real-commit fixtures; write-time refusals in update-epic, record-gate-review, record-reconcile |
| `git-commit-takes-the-whole-index` | subagent brief template (hard constraint) |
| `git-rewinds-restore-tracked-conductor-state` | habit; scripts/test/commit-observation.test.mjs 5.3 asserts the attribution is present before observing, as the worked example |
...
| `narrowing-a-guard-retests-what-it-protected` | scripts/test/verb-surface.test.mjs DISPATCH_BASELINE help sweep |
...
| `stacked-background-commits-collide-on-the-lock` | habit — the wait-then-verify wrapper at `scratchpad/final-commit.sh`; pm's own pre-commit hook already holds a suite lock and prints "another worktree is running the suite — waiting", which is the mechanism this lesson wants and which does NOT cover the index lock. detect matcher on a `git commit` backgrounded with `&`; `run_in_background` stays a habit. |
