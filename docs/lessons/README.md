...
| [`an-uncertified-module-change-skips-the-functional-half`](an-uncertified-module-change-skips-the-functional-half.md) | You are about to commit a change to an engine module that is NOT in `certifiedModules()` — a refusal's wording, a printed count, a message — and the drift script asks only for `certify.mjs sweeps`, so the functional half will not run before the commit. | Before committing any change to text an uncertified module prints, `rg` the functional half (scripts/test/functional/) for the old wording and run every file it hits. The per-commit gate runs the assertion half and the sweeps; it runs the functional half only for a certified module, so a functional test that pins an uncertified module's output is checked by nothing until the next functional certify. | — |
...
| `an-uncertified-module-change-skips-the-functional-half` | habit — no mechanism. The drift script's freshness check keys on certifiedModules(); widening it to every module a functional test observes is the product fix, filed as #229. |
