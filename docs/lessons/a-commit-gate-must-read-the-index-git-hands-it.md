...
cost: pm's pre-commit hook ran the suite over the WORKING TREE from 0.16.0 to 0.49.0, so a failing test that was staged, with a passing copy left unstaged, committed green while HEAD held the failure (code review 0.43.0, E2). Fixing it exposed a second hole of the same kind: the hook unset GIT_INDEX_FILE before reading the index, and under `git commit -a` and `git commit -- <paths>` git hands the hook a different index (`.git/index.lock`, `.git/next-index-<pid>.lock`) while `.git/index` is stale. So the drift check and the floor's count read the wrong index for the commit form `git-commit-takes-the-whole-index` RECOMMENDS. Both holes passed every gate for 33 releases, because a gate that verifies the wrong bytes still produces a green result.
