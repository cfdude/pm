...
* **`verify-worktrees` reports the whole path of a worktree whose name holds a line feed.** It read
  ...
  Where git cannot list worktrees for any reason but "not a git repository" — an older git,
  dubious ownership, a corrupt repository, git missing — it now refuses with git's own message
