...
- The `.claude/skills/dogfooding` and `.claude/skills/lessons` copies are now **stubs redirecting
  ...
  hook like any other user. Note the interim: `PM_ENGINE_DELEGATION` redirects which *engine*
  executes, and cannot register a hook, so `hooks/hooks.json` still comes from the INSTALLED
  plugin. This repo therefore has no lesson advisor from this commit until the release ships and
  is installed — the same gap #126's move accepted, and the price of one live copy instead of two.
