...
the epic. Additive, idempotent, `state`-only, scoped to the backfill's own token. Note that until
`plugin.json` reaches `0.32.0` this entry replays on each `/pm:upgrade` (it is guarded on an
absent `registeredBy`, so a replay is a no-op) — **this change must ship in 0.32.0 or later.**
