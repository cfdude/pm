* **The "tests in the engine" number on pm-plugin.dev comes only from a clean run, and the run is
  ...
  and publishes nothing unless the runner exited 0 with `fail 0`, `cancelled 0` and more than zero
  tests. A failed run is recorded against #219 before it is re-run, so a flake cannot be retried
  away silently. The flaky test #219 reported is not identified yet; this makes its next occurrence
  diagnosable.
