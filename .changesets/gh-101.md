...
No migration: effective priority is computed and never stored, and `rank` is a purely additive
optional field whose absence is a legal, meaningful value (sorts last). There is nothing in an
existing `state.json` to transform, and a state file written by the prior version loads and
orders exactly as it did.
